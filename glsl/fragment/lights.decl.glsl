...
vec3 PointLight_specular(PointLight self, vec3 point, vec3 normal, vec3 direction, int n) {
    vec3 vector_to_light = self.position - point;
    float cosine_term = dot(normalize(vector_to_light), normal);
    return color * pow(self.intensity, 0.3) * pow(max(dot(normal * cosine_term * 2 - normalize(vector_to_light), -direction), 0.f), n);
}
...
vec3 DirectionalLight_specular(DirectionalLight self, vec3 point, vec3 normal, vec3 direction, int n) {
    return vec3(0);
}
...
vec3 GlobalLight_specular(GlobalLight self, vec3 point, vec3 normal, vec3 direction, int n) {
    return vec3(0);
}
